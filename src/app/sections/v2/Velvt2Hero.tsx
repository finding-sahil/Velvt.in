...
export function Velvt2Hero({
  title = "VELVT",
  tagline = "It starts as a thought, ends as a memory.",
  sub = "Immersive nightlife · Silchar, Assam",
  primaryCta = "Explore Curse 2.O",
  primaryHref = "/events/velvt-curse-2-o",
  secondaryCta = "Book Passes",
  secondaryHref = "/tickets",
}: Velvt2HeroProps) {
  // Always enforce clean, compacted subtitle (never allow long verbose sentences)
  const compactSub =
    !sub ||
    sub.includes("Thematic nightlife") ||
    sub.includes("sensory productions")
      ? "Immersive nightlife · Silchar, Assam"
      : sub;
  ...
  return (
    <section className="relative min-h-[78vh] flex items-center justify-center text-center pt-24 pb-14 sm:py-24 px-4 overflow-hidden">
      {/* Signature Atmospheric Ambient Lighting Halos */}
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto flex flex-col items-center px-2 sm:px-4">
        {/* Main Brand Title — Bold & Commanding at the TOP */}
        ...
        {/* Cinematic Tagline — Directly below headline */}
        <p className="hero-tagline text-white animate-fade-in-up animation-delay-200">
          {tagline}
        </p>

        {/* Compacted Subtitle — Clean 4-word statement sitting below tagline */}
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 w-full sm:w-auto animate-fade-in-up animation-delay-400">
          ...
          <Button href={secondaryHref} variant="secondary" size="md" className="w-full sm:w-auto">
            {secondaryCta}
          </Button>
